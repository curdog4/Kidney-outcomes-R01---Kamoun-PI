...
(Patients <- Patients_and_donors %>% filter(PatientTypeCd == "Patient", firstnm != "", lastnm != "") %>% select(-UNOSId))
(Donors <- Patients_and_donors %>% filter(PatientTypeCd == "Donor"))
```
```{r}
# Import transplanted kidney patients from TIEDI record.
(TIEDI_record_2017_2024 <- read_rds("MDJH_TEIDI_combined_2017_2024.rds"))
```
```{r}
...
#(TIEDI_record_2017_2024_patients <- TIEDI_record_2017_2024 %>% left_join(Patients, by = c("SSN.recipient" = "SSNbr"), relationship = "many-to-one"))
#(TIEDI_record_2017_2024_patients_fullSSN <- TIEDI_record_2017_2024_patients %>% filter(!is.na(PatientId)))

# Filter for TIEDI records that won't match to HistoTrac records. 
(TIEDI_record_2017_2024_unnmatched <- TIEDI_record_2017_2024 %>% anti_join(Patients, by = c("SSN.recipient" = "SSNbr")))

# Filter for TIEDI records that will match
(TIEDI_record_2017_2024_will_match <- TIEDI_record_2017_2024 %>% semi_join(Patients, by = c("SSN.recipient" = "SSNbr")))

# Now match recipients from the TIEDI record with the HistoTrac database. The relationship below is set to "many-to-one" because some recipients will have had multiple transplants. If this gives an error there is a problem.
(TIEDI_record_2017_2024_patients_full_SSN <- TIEDI_record_2017_2024_will_match %>% left_join(Patients, by = c("SSN.recipient" = "SSNbr"), relationship = "many-to-one"))
...
(TIEDI_record_2017_2024_full_name <- TIEDI_record_2017_2024_unnmatched %>% mutate(full_name = str_c(first.recipient, " ", last.recipient), last_4 = str_extract(SSN.recipient, "[:digit:]{4}$"), .after = center))
...
(TIEDI_record_2017_2024_patients_fuzzy <- TIEDI_record_2017_2024_full_name 
...
```
```{r}
# Combine the patients from the full SSN matching and the fuzzy matching
(TIEDI_record_2017_2024_patients <- TIEDI_record_2017_2024_patients_full_SSN %>% bind_rows(TIEDI_record_2017_2024_patients_fuzzy) %>% arrange(Tx_date))
...
(TIEDI_record_2017_2024_patients_IDs <- TIEDI_record_2017_2024_patients %>% mutate(study_ID = str_c(params$Center_ID, "-", histocompatibility_ID), .before = histocompatibility_ID))
...
# *** NOTE: JHU uses UNOS ID for decease donor last name, so the below logic is broken. Remove is.na(last_donor) or change to test last_donor == donor_ID. ***
# First look for deceased donors that won't match joining by UNOS ID.
#TIEDI_record_2017_2024_patients_IDs %>% filter(is.na(last_donor)) %>% anti_join(Donors, by = c("donor_ID" = "UNOSId")) # Go through this list to see if UNOS IDs for deceased donors need to be updated in HistoTrac.
TIEDI_record_2017_2024_patients_IDs %>% filter(is.na(donor_ID)) %>% anti_join(Donors, by = c("donor_ID" = "UNOSId")) # 
...
#TIEDI_record_2017_2024_patients_IDs %>% filter(is.na(last_donor)) %>% left_join(Donors, by = c("donor_ID" = "UNOSId"), suffix = c(".recipient", ".donor")) %>% filter(n() > 1, .by = "histocompatibility_ID") # If this table has entries, resolve the issues.
TIEDI_record_2017_2024_patients_IDs %>% filter(is.na(donor_ID)) %>% left_join(Donors, by = c("donor_ID" = "UNOSId"), suffix = c(".recipient", ".donor")) %>% filter(n() > 1, .by = "histocompatibility_ID")

# Now perform a final join for the deceased donors, using "many-to-one" joining.
#(TIEDI_record_2017_2024_patients_donors_deceased <- TIEDI_record_2017_2024_patients_IDs %>% filter(is.na(last_donor)) %>% left_join(Donors, by = c("donor_ID" = "UNOSId"), suffix = c(".recipient", ".donor"), relationship = "many-to-many"))
(TIEDI_record_2017_2024_patients_donors_unosid <- TIEDI_record_2017_2024_patients_IDs %>% filter(!is.na(donor_ID)) %>% left_join(Donors, by = c("donor_ID" = "UNOSId"), suffix = c(".recipient", ".donor"), relationship = "many-to-many"))
```
...
#living_donor_recipients <- TIEDI_record_2017_2024_patients_IDs %>% filter(!is.na(last_donor)) %>% pull(PatientId)
no_unosid_donor_recipients <- TIEDI_record_2017_2024_patients_IDs %>% filter(is.na(donor_ID)) %>% pull(PatientId)
...
(HT_donors <- tbl(con, "PatientCase") 
  ...
  %>% filter(PatientId %in% no_unosid_donor_recipients)
...
#(TIEDI_record_2017_2024_patients_donors_living_test <- TIEDI_record_2017_2024_patients_IDs 
(TIEDI_record_2017_2024_patients_donors_no_unosid_test <- TIEDI_record_2017_2024_patients_IDs 
...
#TIEDI_record_2017_2024_patients_donors_living_test %>% select(HospitalID.recipient, firstnm.recipient, lastnm.recipient, Tx_date, full_name_TIEDI, RelatedPatientID, full_name_HT, donor_name_match) %>% rename(HT_ID.donor = RelatedPatientID) %>% arrange(desc(donor_name_match)) # This is arranged with the worst name matches at the top. Look through to see if anything seems amiss.
TIEDI_record_2017_2024_patients_donors_no_unosid_test %>% select(HospitalID.recipient, firstnm.recipient, lastnm.recipient, Tx_date, full_name_TIEDI, RelatedPatientID, full_name_HT, donor_name_match) %>% rename(HT_ID.donor = RelatedPatientID) %>% arrange(desc(donor_name_match))

```

```{r}
# Once everything looks good, clean up the living donor table and merge the deceased and living donor tables.
#(TIEDI_record_2017_2024_patients_donors_living <- TIEDI_record_2017_2024_patients_donors_living_test %>% select(-full_name_TIEDI, -full_name_HT, -donor_name_match) %>% rename(PatientId.recipient = PatientId, PatientId.donor = RelatedPatientID))
#(TIEDI_record_2017_2024_patients_donors_no_unosid <- TIEDI_record_2017_2024_patients_donors_no_unosid_test %>% select(-full_name_TIEDI, -full_name_HT, -donor_name_match) %>% rename(PatientId.recipient = PatientId, PatientId.donor = RelatedPatientID))

# Final merger of all donors.
#(TIEDI_record_2017_2024_patients_donors <- TIEDI_record_2017_2024_patients_donors_deceased %>% bind_rows(TIEDI_record_2017_2024_patients_donors_living) %>% arrange(Tx_date) %>% select(-UNOSId.recipient, -UNOSId.donor))
#(TIEDI_record_2017_2024_patients_donors <- TIEDI_record_2017_2024_patients_donors_unosid %>% bind_rows(TIEDI_record_2017_2024_patients_donors_no_unosid) %>% arrange(Tx_date) %>% select(-UNOSId.recipient, -UNOSId.donor))
(TIEDI_record_2017_2024_patients_donors <- TIEDI_record_2017_2024_patients_donors_unosid %>% arrange(Tx_date))
```
# NOTE: Stopped here w/NB. Research why below donors not matching.
```{r}
# Inspect the final table with recipient and donor typing. 

# Determine how many donor entries are missing after the join.
final_TIEDI_cases <-  TIEDI_record_2017_2024_patients_donors %>% pull(histocompatibility_ID)
#tmp_missing_unresolved <- TIEDI_record_2017_2024_patients %>% filter(!histocompatibility_ID %in% final_TIEDI_cases) # Determine if you want to resolve any of these.
#write.xlsx(tmp_missing_unresolved, "Missing_and_Unresolved_Records.xlsx")
```

```{r}
# How many donors don't have typing?
TIEDI_record_2017_2024_patients_donors %>% count(A1Cd.donor) # All donors should have typing.
...
(Study_IDs <- TIEDI_record_2017_2024_patients_donors %>% select(histocompatibility_ID, PatientId.recipient, donor_ID, PatientId.donor) %>% mutate(study_ID = str_c(params$Center_ID, "-", histocompatibility_ID)))
```

```{r}
# Clean the main recipient and donor typing table of PHI and save for transfer to Penn

(MDJH_main_table_2017_2024 <- TIEDI_record_2017_2024_patients_donors %>% select(study_ID, histocompatibility_ID, donor_ID, A1Cd.recipient:mDPB12EqCd.recipient, A1Cd.donor:mDPB12EqCd.donor))

MDJH_main_table_2017_2024 %>% saveRDS("Files/MDJH_main_table_2017_2024.rds")
MDJH_main_table_2017_2024 %>% write_csv("Files/MDJH_main_table_2017_2024.csv.gz")
```
...
PatientId_recipient <- TIEDI_record_2017_2024_patients_donors %>% pull(PatientId.recipient)

PatientId_donor <- TIEDI_record_2017_2024_patients_donors %>% pull(PatientId.donor)
...
(Study_IDs_HistoTrac_IDs_patients <- Study_IDs %>% select(study_ID, PatientId.recipient))
(Study_IDs_HistoTrac_IDs_donors <- Study_IDs %>% select(donor_ID, PatientId.donor))
...
(Study_IDs_Tx_dates <- TIEDI_record_2017_2024_patients_donors %>% select(histocompatibility_ID, PatientId.recipient, Tx_date) %>% mutate(study_ID = str_c(params$Center_ID, "-", histocompatibility_ID)))
```
...
```
```{r}
# *** OMITTED TESTING THIS CHUNK, TABLE EXISTS, SCHEMA DOES NOT  MATCH ***
...
# *** NOTE: STOPPED HERE. RT-PCR (SURETYPER) XML FILES NOT STORED IN STANDARD LOCATION PRIOR TO JANUARY 2021 (HT GO-LIVE) ***
```{r}
